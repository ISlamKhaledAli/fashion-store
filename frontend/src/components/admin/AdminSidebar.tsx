...
import { LayoutDashboard, ShoppingCart, Package, Users, BarChart3, Settings, LucideIcon } from "lucide-react";
    ...
    <aside className="fixed left-0 top-0 h-screen w-64 bg-primary border-r border-white/5 flex flex-col z-60">
      ...
      <nav className="flex-1 space-y-1 px-4">
        {links.map((link) => (
          ...
          >
            <link.icon className="mr-3 text-lg" size={18} strokeWidth={1.5} />
            {link.name}
          </Link>
        ))}
      </nav>
