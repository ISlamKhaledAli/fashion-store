...
const CheckoutForm = ({
    ...
    try {
      ...
      try {
        ...
        setError(
          error.response?.data?.message ||
            "Order creation failed. Please check your cart."
        );
        setLoading(false);
        return;
      }
      ...
      if (
        ...
        try {
          ...
          console.error(
            "Failed to acknowledge payment success on frontend:",
            err
          );
        }
  ...
  return (
    <form
      ...
      className="animate-in fade-in slide-in-from-right-8 space-y-8 duration-700"
    >
      ...
      {error && (
        <p className="text-sm font-medium tracking-wider text-error uppercase">
          {error}
        </p>
      )}
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4 py-24">
        ...
        <p className="text-[10px] font-medium tracking-[0.2em] text-on-surface-variant uppercase">
